...
**Catálogo de golpes data-driven** (`golpes.json`): todos os golpes do jogo, com dano, frames e **usos limitados** para golpes não-físicos (ex.: Bola de Fogo 70, Lança-Chamas 30); golpes físicos são infinitos. Feras **aprendem golpes por nível** (tabela de aprendizado por espécie), preenchendo até **4 slots** acionados por **Z / X / C / V** (slot 1 = golpe físico inicial, intocável). Ao aprender com os slots cheios, o novo substitui um antigo; golpes esquecidos podem ser **relembrados** no menu de status (fora de batalha). **Golpe forte por MODIFICADOR** (decidido, substituindo a carga de 1,5s, que substituíra os combos de direção — combos ficam para o futuro): toque no botão = golpe simples; **SHIFT + botão** solta a **versão forte** daquele slot na hora (Investida→Investida Feroz, Bola de Fogo→Lança-Chamas, ~2x de dano total). **F** lança o Cristal de Captura. Golpes se restauram no Centro de Curas (tudo), ao subir de nível (20%) ou com itens (futuro).

**Vantagem de tipo** (decidido, tabela em `tipos.json`): fogo>planta, água>fogo, planta>água, elétrico>água — golpe com vantagem causa x1,5 ("Super efetivo!"), na contramão x0,75. **Energia (ki) v2 — economia estilo elixir** (decidido): a barra começa na **METADE (50)**; especial simples gasta 10, versão forte 60. Recuperação por três vias: **regeneração passiva lenta** (2/s — a luta nunca trava), **carga ativa segurando A** (16/s, parado e vulnerável — quem apanha carregando toma **+25% de dano**; após 45s de luta a "fúria do crepúsculo" dobra a carga) e **golpe físico certeiro gera +12** (estilo Hollow Knight — até os magos querem entrar no corpo-a-corpo para reabastecer, e feras físicas são baterias ambulantes). O golpe supremo foi REMOVIDO por ora (pode voltar como mecânica futura); os especiais vivem em X/C/V + Shift. Movimento é SÓ nas setas.

**Usos** (decidido): golpes especiais têm um POTE de 10 usos por golpe-base; a versão simples gasta 1 e a versão forte gasta 2 **do mesmo pote** (Bola de Fogo e Lança-Chamas dividem os 10). O pote cresce +1 por nível a partir do 11 (Lv.11 = 11, Lv.12 = 12...). A IA administra a própria barra e também carrega. **Golpes de FEIXE** (decidido): Trovão (elétrico) e Raio Esmeralda (planta) envolvem a fera numa aura e cravam um raio instantâneo no alvo (alcance 8). **Arenas por bioma** (decidido): o ringue veste o bioma do mapa, e biomas podem ter PERIGOS — no deserto, cactos na cerca espetam quem encosta. **Feras voadoras** (`voa` nos dados) têm salto duplo/triplo. Defesa/bloqueio: adiado. Feras alfa: descartado (evoluções cumprem esse papel).

**Captura** (regra decidida): cristal esférico lançado por trás da fera (do lado do treinador); só com o alvo a ≤50% de vida — chance 50% na metade, subindo até 100% a 25% de vida, multiplicada pelo fator de raridade (comum x1, rara x0,6, muito rara x0,25). Feras de TREINADOR não podem ser capturadas.

**Interação e conversa** (decidido): **Z** fala com o que estiver perto (uma pílula "Z — ..." avisa): moradores têm falas, **placas** dão direções, a **enfermeira** do Centro de Curas cura a equipe ao conversar (nada de cura automática por proximidade), e **treinadores** desafiam pela conversa — o desafio **não pode ser recusado** na primeira vez.

**Treinadores NPC** (decidido, dados em `mapas.json` › `treinadores`): desafiantes com equipes fixas espalhados pelo mundo (Rui Andarilho na Trilha, Beto das Rotas no Campo, Maga Vilma na Rota); falar com um puxa o duelo, as feras deles entram em sequência, vencer marca o treinador como derrotado na sessão. São o embrião do sistema de insígnias.
